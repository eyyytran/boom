...
export default function Boom() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, "rooms", game.state.roomId), doc => {
      ...
      dispatch(modal.action.setIsShowIsTurnModal(dbGameState.gameStarted && dbGameState.whosTurn === game.state.playerNum));
      dispatch(game.action.setIsTurn(dbGameState.gameStarted && dbGameState.whosTurn === game.state.playerNum));
      dispatch(game.action.setWhosTurn(dbGameState.whosTurn));

      if (JSON.stringify(game.state.players) !== JSON.stringify(dbGameState.players)) getParticipants();
  ...
  }, [dispatch, game.action, game.state.playerNum, game.state.players, game.state.roomId, modal.action, user.state.userName]);
    ...
    <Component id="Boom">
      <div className={`${styles.static}`}>
        <div className="flex flex-col justify-start h-full">
          ...
          <div className="flex h-full overflow-y-clip overflow-x-auto snap-x snap-mandatory no-scrollbar scroll-smooth border-4 border-blue-500">
            <Gallery galleryRef={galleryRef} className="shrink-0 border border-red-500 snap-center" />
            <Artboard artboardRef={artboardRef} className="shrink-0 lg:shrink border border-red-500 snap-center" />
            <Chat chatRef={chatRef} className="shrink-0 border border-red-500 snap-center" />
          </div>
          <Navbar galleryButtonRef={galleryButtonRef} artboardButtonRef={artboardButtonRef} chatButtonRef={chatButtonRef} exitButtonRef={exitButtonRef} className="shrink-0" />
        </div>
      </div>
    </Component>
  );
