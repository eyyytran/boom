  ...
  useEffect(() => {
    ...
    const reassignPlayerNum = (playerList: IParticipant[]) => {
      const index = playerList.findIndex(
        (player) => player.player === user.state.userName
      );
    ...
    const unsubscribe = onSnapshot(
      doc(db, "rooms", game.state.roomId),
      (doc) => {
        const data = doc.data();
        ...
        dispatch(
          modal.action.setIsShowIsTurnModal(
            dbGameState.gameStarted &&
              dbGameState.whosTurn === game.state.playerNum
          )
        );
        dispatch(
        ...
        if (
          JSON.stringify(game.state.players) !==
          JSON.stringify(dbGameState.players)
        ) {
          getParticipants();
        ...
        dispatch(
          game.action.setWinner(
            dbGameState.winner ? dbGameState.winner.player : null
          )
        );
