...
export const createOffer = async (peerConnection, receiverId, createdID) => {
  ...
  peerConnection.onicecandidate = event => {
    event.candidate && currentParticipantRef.child("offerCandidates").push({ ...event.candidate.toJSON(), userId: createdID });
  };
...
const createAnswer = async (otherUserId, userId) => {
  ...
  pc.onicecandidate = event => {
    event.candidate && participantRef1.child("answerCandidates").push({ ...event.candidate.toJSON(), userId: userId });
  };
